...
def rotate_gif():
    ...
    # Change GIF every 14 seconds globally
    current_index = int(time.time() / 14) % len(images)

    # Current GIF
    current_gif = images[current_index]

    # Add changing timestamp to bypass cache
    separator = "&" if "?" in current_gif else "?"

    return redirect(
        f"{current_gif}{separator}t={int(time.time())}",
        code=302
    )
